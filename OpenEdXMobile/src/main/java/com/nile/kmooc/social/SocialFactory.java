...
import com.nile.kmooc.social.facebook.FacebookAuth;
import com.nile.kmooc.social.google.GoogleOauth2;
import com.nile.kmooc.social.naver.NaverAuth;
import com.nile.kmooc.social.kakao.KakaoAuth;
...
public class SocialFactory {
    ...
    public static enum SOCIAL_SOURCE_TYPE {
        TYPE_UNKNOWN(-1, "unknown"), TYPE_GOOGLE(100, "google-oauth2"), TYPE_FACEBOOK(101,"facebook"), TYPE_NAVER(102,"naver"), TYPE_KAKAO(103,"kakao");
        ...
        public static SOCIAL_SOURCE_TYPE fromString(String source){
            ...
            if ( "naver".equalsIgnoreCase( source ) )
                return TYPE_NAVER;
            if ( "kakao".equalsIgnoreCase( source ) )
                return TYPE_KAKAO;
    ...
    public static ISocial getInstance(Activity activity, SOCIAL_SOURCE_TYPE type, Config config) {
        ...
        } else if (type == SOCIAL_SOURCE_TYPE.TYPE_KAKAO) {
            if ( isSocialFeatureEnabled(activity.getApplicationContext(),type, config)) {
                return new KakaoAuth(activity);
            }
            else {
                return new ISocialEmptyImpl();
            }
    ...
    public static boolean isSocialFeatureEnabled(Context context, SOCIAL_SOURCE_TYPE type, Config config) {
        ...
        if (type == SOCIAL_SOURCE_TYPE.TYPE_GOOGLE) {
            ...
            return config.getKakaoConfig().isEnabled();
        }
        return true;
    }
